	...
	public UserInfo getUserInfoFromServer(String userId) {
		...
		try {
			...
			if (doc.select("td").size() != 0) {
				...
				/*
				 * IFme (Mico) ¹²ÉÏÕ¾ 682 ´Î£¬·¢±íÎÄÕÂ 70 Æª [´¦Å®×ù]ÉÏ´ÎÔÚ [Tue May 10 21:31:32
				 * 2016] ´Ó [112.21.166.24] µ½±¾Õ¾Ò»ÓÎ¡£ ĞÅÏä£º[¡Ñ] ¾­ÑéÖµ£º[562](ÖĞ¼¶Õ¾ÓÑ)
				 * ±íÏÖÖµ£º[38](ºÜºÃ) ÉúÃüÁ¦£º[368]¡£ Ä¿Ç°²»ÔÚÕ¾ÉÏ, ÉÏ´ÎÀëÕ¾Ê±¼ä [(²»Ïê)]
				 */
				...
				if (m.find()) {
					...
					if(result2.contains("1;36m")){//ÄĞ
						info.gender="ÄĞ";
					}else if(result2.contains("1;35m")){//Å®
						info.gender="Å®";
					}else{//Î´Öª
						System.out.println("ĞÔ±ğ£ºÎ´Öª");
						info.gender="Î´Öª";
					}
					LogUtil.d("´ÓÍøÉÏ»ñÈ¡ÓÃ»§Êı¾İ£º"+info.id);
					return info;
				}
